...
const Messagemodal = ({ show, onClick, dark }: ModalProp) => {
    return (
        <div className={` w-[100%] ${dark ? "text-white" : "text-slate-400"} z-50 backdrop-blur-[10px] left-0 h-[100vh] ${show ? "flex" : "hidden"} justify-center items-center absolute top-0 `}>
            <div className={`w-[600px] flex justify-center items-center phone:w-[100%] phone:h-[250px] h-[300px] ${dark ? "bg-slate-800" : "bg-white"} shadow-2xl rounded-[20px]`}>
                ...
                <h1 className=" phone:text-[30px] font-semibold text-[40px] ">No message yet</h1>
            </div>
        </div>
    )
}
